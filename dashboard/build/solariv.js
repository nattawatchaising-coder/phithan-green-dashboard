const IV_Q = 1.602176634e-19;
const IV_K = 1.380649e-23;
const IV_T0 = 298.15;
const IV_PANEL_EXTRA = {
  tcIsc: 0.045
};
...
function ivAt(par, panel, G, Tc, ka) {
  const g = Math.max(1, scNum(G, 1000)) / 1000,
    dT = scNum(Tc, 25) - 25;
  const Ki = scNum(panel.tcIsc, IV_PANEL_EXTRA.tcIsc) / 100 * scNum(panel.isc);
  const Kv = scNum(panel.tcVoc, SC_PANEL_EXTRA.tcVoc) / 100 * scNum(panel.voc);
...
function ivFamily(par, panel, o) {
  ...
  const ns = Math.max(1, Math.round(scNum(o.nSeries, 1))),
    np = Math.max(1, Math.round(scNum(o.nPar, 1)));
  const kD = o.derate == null ? 1 : o.derate;
  const temp = o.mode === "temp";
  const list = temp ? (o.temps || IV_TLEVELS).map(t => ({
    g: scNum(o.g, 1000),
    tc: t
  })) : (o.levels || IV_GLEVELS).map(g => ({
    g,
    tc: scNum(o.tc, 25)
  }));
  return list.map(c => {
...
function ivString(par, panel, G, Tc, nSeries, nPar, derate) {
  ...
  const ns = Math.max(1, Math.round(nSeries || 1)),
    np = Math.max(1, Math.round(nPar || 1));
  const kD = derate == null ? 1 : derate;
  return {
    nSeries: ns,
    nPar: np,
    g: m.g,
    ...
    pts: m.pts.map(q => ({
      v: scR(q.v * ns, 2),
      i: scR(q.i * np * kD, 3),
      p: scR(q.p * ns * np * kD, 1)
    })),
    isc: scR(m.isc * np * kD, 2),
    voc: scR(m.voc * ns, 1),
    vmp: scR(m.vmp * ns, 1),
    imp: scR(m.imp * np * kD, 2),
    pmax: scR(m.pmax * ns * np * kD, 1),
    ff: scR(m.ff * 100, 1)
  };
}
...
function ivIrradiance(o) {
  ...
  const bT = scNum(o.tilt, 0) * SC_DEG,
    cosB = Math.cos(bT),
    sinB = Math.sin(bT);
  ...
  const refl = ghi * scNum(o.albedo, SC_ENV.albedo) * (1 - cosB) / 2;
  ...
  return {
    ...
    month: mIdx,
    ghi: scR(ghi, 0),
    ghiClear: scR(ghiClear, 0),
    dni: scR(dni, 0),
    dhi: scR(dhi, 0),
    kt: scR(kt, 2),
    aoi: scR(Math.acos(scClamp(cosAoi, 0, 1)) / SC_DEG, 1),
    cosAoi: scR(cosAoi, 3),
    iam: scR(iam, 3),
    beam: scR(beam, 0),
    diff: scR(diff, 0),
    ...
    poa: scR(poa, 0),
    poaNet: scR(poaNet, 0),
    shadeLoss: scR(poa - poaNet, 0),
    tiltGain: ghi > 0 ? scR(poa / ghi, 3) : 0,
    measured,
    night: false
  };
}
...
function ivExpect(panel, par, cond, nSeries, nPar, ageYears) {
  ...
  if (s) {
    s.derate = scR(derate * 100, 1);
    s.ageYears = yr;
  }
  return s;
}
function ivToStc(meas, panel, par, nSeries) {
  const G = scNum(meas.g),
    Tc = scNum(meas.tmod, 25),
    dT = Tc - 25;
  if (!(G > 0)) return null;
  const kG = 1000 / G,
    ns = Math.max(1, Math.round(nSeries || 1));
...
function ivDiagnose(stc, exp, panel, nSeries) {
  ...
  if (rV && rV < 0.97) {
    ...
    F.push({
      ...
      why: miss >= 1 ? "หายไปประมาณ " + miss + " แผง (" + scR(miss * vMod, 0) + " V) — เท่ากับมีแผงหลุดวงจร ต่อขาดไป หรือไดโอดบายพาสลัดวงจรอยู่" : "อาจนับจำนวนแผงในสตริงคลาดเคลื่อน หรือมีจุดต่อหลวมบางจุด",
    ...
    F.push({
      k: "voc",
      sev: "warn",
      t: "แรงดันวงจรเปิดสูงกว่าที่ควรได้ " + pct(rV) + "%",
  ...
  if (rP) {
    if (rP >= 0.95) F.push({
      k: "ok",
      sev: "good",
      t: "กำลังที่วัดได้ " + scR(rP * 100, 1) + "% ของที่ควรได้",
      why: "อยู่ในเกณฑ์ปกติของการตรวจรับงาน (ทั่วไปยอมรับที่ ≥ 95% หลังชดเชยสภาพอากาศแล้ว)",
      do: ""
    });else F.push({
      k: "pmax",
      sev: rP < 0.85 ? "bad" : "warn",
...
function ivAssess(o) {
  ...
  const expStc = ivExpect(panel, par, {
    g: 1000,
    tc: 25
  }, o.nSeries, o.nPar, o.ageYears);
  const has = scNum(meas.voc) > 0 || scNum(meas.isc) > 0 || scNum(meas.pmax) > 0;
  const stc = has ? ivToStc(Object.assign({}, meas, {
    g: gUsed,
    tmod: tUsed
  }), panel, par, o.nSeries) : null;
  ...
  return {
    ...
    ratio,
    hasMeas: has,
    gSource: scNum(meas.g) > 0 ? "meas" : "model",
    tSource: meas.tmod != null && meas.tmod !== "" ? "meas" : "model"
  };
}
...
function ivHitTri(o, d, a, e1, e2) {
  const px = d.y * e2.z - d.z * e2.y,
    py = d.z * e2.x - d.x * e2.z,
    pz = d.x * e2.y - d.y * e2.x;
...
function ivHitWall(o, d, poly, top) {
  ...
  const p0 = [o.x + d.x * t0, o.z + d.z * t0],
    p1 = [o.x + d.x * t1, o.z + d.z * t1];
...
function ivGeom(st) {
  const foot = typeof p3FootAll === "function" ? p3FootAll(st) : {
    panels: [],
    outlines: []
  };
  ...
  (st.roofs || []).forEach(roof => {
    ...
    surfs.forEach(s => {
      ...
      cz /= s.pts.length;
      s.pts.forEach(q => {
        rad = Math.max(rad, Math.hypot(q.x - cx, q.y - cy, q.z - cz));
      });
      const tris = [];
    ...
    if (hull.length >= 3 && top > 0.2) {
      ...
      cz /= poly.length;
      poly.forEach(q => {
        rad = Math.max(rad, Math.hypot(q[0] - cx, q[1] - cz));
      });
      blockers.push({
        t: "w",
        poly,
        ...
        br: Math.hypot(rad, top / 2),
        name: "ตัวอาคาร " + (roof.name || "")
      });
    }
  });
  (st.obstacles || []).forEach(o => {
    ...
    if (o.kind === "tree") {
      ...
      const w = Math.max(0.2, scNum(o.w, 1)) / 2,
        d = Math.max(0.2, scNum(o.d, 1)) / 2;
      const c = {
        x,
        y: h / 2,
        z
      ...
      blockers.push({
        ...
        bc: c,
        br: Math.hypot(w, h / 2, d),
        name: o.name || "สิ่งบดบัง",
        id: o.id
      });
    }
  });
  panels.forEach(p => {
    ...
    blockers.push({
      ...
      vv: ivDot(p.v, p.v),
      bc: c,
      br: Math.hypot(p.u.x + p.v.x, p.u.y + p.v.y, p.u.z + p.v.z),
      name: "แผง"
    });
  });
  const S = 0.62;
  ...
  for (let i = 0; i < geo.panels.length; i++) {
    ...
    for (let s = 0; s < S.length; s++) {
      ...
      for (let k = 0; k < B.length; k++) {
        ...
        let hit = false;
        if (b.t === "s") hit = ivHitSphere(o, dir, b.c, b.r);else if (b.t === "b") hit = ivHitBox(o, dir, b.lo, b.hi);else if (b.t === "q") hit = ivHitQuad(o, dir, b);else if (b.t === "w") hit = ivHitWall(o, dir, b.poly, b.top);else if (b.t === "t") {
...
function ivSunDir(alt, az) {
  const a = alt * SC_DEG,
    z = az * SC_DEG;
  return {
    x: Math.sin(z) * Math.cos(a),
    y: Math.sin(a),
    z: -Math.cos(z) * Math.cos(a)
  };
}
function ivShadeAnnual(st, byPanel, groups, o) {
  ...
  const gPan = {};
  geo.panels.forEach(p => {
    const k = byPanel[p.uid];
    if (k && gInfo[k]) (gPan[k] = gPan[k] || []).push(p.uid);
  });
  ...
  let lTot = 0,
    ...
    lGeoTot = 0;
  geo.panels.forEach(p => {
    const t = totW[p.uid] || 0,
      l = lossW[p.uid] || 0;
    byPanelPct[p.uid] = t > 0 ? scR(l / t * 100, 1) : 0;
    lTot += l;
    tTot += t;
    lGeoTot += geoW[p.uid] || 0;
    const gk = byPanel[p.uid];
    if (gk) {
      const a = gAcc[gk] = gAcc[gk] || {
        l: 0,
        t: 0,
        n: 0
      };
  ...
  const byGroup = {};
  Object.keys(gAcc).forEach(k => {
    byGroup[k] = gAcc[k].t > 0 ? scR(gAcc[k].l / gAcc[k].t * 100, 1) : 0;
  });
  const worst = geo.panels.map(p => ({
    uid: p.uid,
    roofName: p.roofName,
    key: p.key,
    pct: byPanelPct[p.uid]
...
function ivShadeMoment(st, byPanel, groups, o) {
  ...
  geo.panels.forEach(p => {
    const gk = byPanel[p.uid];
    if (!gk) return;
    const a = acc[gk] = acc[gk] || {
      s: 0,
      n: 0
    };
    a.s += sh.byUid[p.uid] || 0;
    a.n++;
  });
  const byGroup = {};
  Object.keys(acc).forEach(k => {
    byGroup[k] = scR(acc[k].s / acc[k].n * 100, 1);
  });
  return {
    byUid: sh.byUid,
    byGroup,
    overall: scR(sh.mean * 100, 1),
    sun,
    panels: geo.panels.length,
    obstacles: geo.obstacles,
    night: false
...
const IV_SUNDAYS = [{
  ...
  doy: 110,
  label: "20 เม.ย. · 23 ส.ค."
}, {
  doy: 79,
  label: "20 มี.ค. · 23 ก.ย."
}, {
  doy: 52,
  label: "21 ก.พ. · 23 ต.ค."
}, {
...
function ivAzSplit(pts) {
  ...
  for (let i = 0; i < pts.length; i++) {
    if (i && Math.abs(pts[i].az - pts[i - 1].az) > 180) {
...
function ivSunPath(o) {
  ...
  const paths = IV_SUNDAYS.map(d => {
    ...
    return {
      ...
      pts,
      segs: ivAzSplit(pts),
      rise: pts.length ? pts[0].h : null,
      set: pts.length ? pts[pts.length - 1].h : null,
      peak: pts.reduce((a, q) => !a || q.alt > a.alt ? q : a, null)
    };
  }).filter(p => p.pts.length);
  const hours = [];
  for (let hh = 5; hh <= 19; hh++) {
    const pts = IV_SUNDAYS.map(d => {
      const s = scSunPos(lat, lng, d.doy, hh);
      return s.alt > 1 ? {
        az: s.az,
        alt: s.alt
      } : null;
    }).filter(Boolean);
    if (pts.length > 1) hours.push({
      h: hh,
      pts,
      segs: ivAzSplit(pts)
    });
...
function ivIsoShade(st, o) {
  ...
  const geo = ivGeom(st);
  if (!geo.panels.length) return null;
  const az0 = scNum(o.azMin, 0),
    az1 = scNum(o.azMax, 354);
  const azStep = scNum(o.azStep, 6),
    altStep = scNum(o.altStep, 5);
  const maxP = o.maxPanels != null ? Math.max(6, Math.round(scNum(o.maxPanels, 48))) : scClamp(Math.round(3000 / Math.max(1, geo.blockers.length)), 12, 48);
  let G = geo,
    sampled = 0;
  if (geo.panels.length > maxP) {
    const stride = geo.panels.length / maxP,
      pick = [];
    for (let i = 0; i < maxP; i++) pick.push(Math.min(geo.panels.length - 1, Math.round(i * stride)));
    G = {
      panels: pick.map(i => geo.panels[i]),
      samples: pick.map(i => geo.samples[i]),
      blockers: geo.blockers
    };
...
function ivElecLoss(fracs, o) {
  ...
  fracs.forEach(f => {
    ...
    const sub = Math.min(nD, Math.ceil(f * nD));
    subLost += sub;
    lost += sub / nD * tol;
  });
  const geo = geoSum / n,
...
function ivStringShaded(par, panel, nSeries, o) {
  ...
  const total = ns * nD;
  const fracs = Array.isArray(o.fracs) && o.fracs.length ? o.fracs : new Array(ns).fill(scClamp(scNum(o.shade, 0), 0, 1));
  let hit = 0;
  fracs.forEach(f => {
    if (f * 100 >= E.minFrac) hit += Math.min(nD, Math.ceil(f * nD));
  });
  ...
  for (let k = N; k >= 0; k--) {
    ...
    let V = 0;
    list.forEach(c => {
      V += c.n * (I > c.isc ? -0.7 : ivVatI(c.pts, I));
    });
    if (V < 0) continue;
    pts.push({
      v: scR(V, 2),
...
function ivDaySim(st, panel, groups, byPanel, o) {
  ...
  const wp = scNum(panel.wp, 650),
    alb = scNum(o.albedo, SC_ENV.albedo);
  const noct = scNum(panel.noct, SC_PANEL_EXTRA.noct),
    tcP = scNum(panel.tcPmax, SC_PANEL_EXTRA.tcPmax);
  const kGhi = scNum(o.ghi) > 0 ? null : SC_KC[mIdx];
  const dt = scNum(o.dt, 0) || 0.25;
  const gKw = {};
  groups.forEach(g => {
    gKw[g.key] = g.count * wp / 1000;
  });
  const gPan = {};
  geo.panels.forEach(p => {
    const k = byPanel[p.uid];
    if (k) (gPan[k] = gPan[k] || []).push(p.uid);
  });
  const rows = [];
  let sunrise = null,
    sunset = null,
    ...
    dayKwh = 0,
    dayKwhNoShade = 0;
  const acKw = scNum(o.acKw, 0),
    dcLoss = scNum(o.dcLoss, 1),
    invEff = scNum(o.invEff, 97.5) / 100;
  for (let h = 4; h <= 20.001; h += dt) {
    const sun = scSunPos(lat, lng, doy, h);
    if (sun.alt <= 0) {
      if (sunrise != null && sunset == null) sunset = h;
      continue;
    }
  ...
  return {
    ...
    month: mIdx,
    tAmb: tAmb0,
    sunrise: sunrise == null ? null : scR(sunrise, 2),
    sunset: sunset == null ? null : scR(sunset, 2),
    peak,
    bestHour: best ? best.row.h : 12,
    dayKwh: scR(dayKwh, 1),
...
function ivYearSim(st, panel, groups, byPanel, o) {
  ...
  for (let m = 0; m < 12; m++) {
    ...
    const sim = ivDaySim(st, panel, groups, byPanel, Object.assign({}, o, {
      date,
      dt: 0.5,
      geo
    }));
    ...
    months.push({
      ...
      days,
      dayKwh: sim.dayKwh,
      monthKwh: scR(sim.dayKwh * days, 0),
      shadeLossPct: sim.shadeLossPct,
      shadeLossKwh: scR(sim.shadeLossKwh * days, 1),
      maxPoa: sim.maxPoa,
      peakAc: sim.peak ? sim.peak.ac : 0,
      peakAt: sim.peak ? sim.peak.h : null,
      sunrise: sim.sunrise,
      sunset: sim.sunset,
      bestHour: sim.bestHour,
      shadeFrom: sim.shadeFrom,
      shadeTo: sim.shadeTo,
      clipHours: sim.clipHours
    });
  }
  ...
  return {
    ...
    shadeLossKwh: Math.round(lossKwh),
...
function ivRoi(annual, panel, dcKw, r, x) {
  ...
  for (let y = 1; y <= yrs; y++) {
    ...
    if (bRepY > 0 && bRepCost > 0 && y < yrs && y % bRepY === 0) {
      rep += bRepCost;
      bAge = 0;
    }
    ...
    rows.push({
      ...
      net: Math.round(net),
      cum: Math.round(cum),
      selfPct: scR(fSelf * 100, 1),
      bKeep: scR(bKeep * 100, 0)
    });
  }
  ...
  let dCost = capex,
    dKwh = 0;
  rows.forEach((q, i) => {
    dCost += (q.om + q.rep) / Math.pow(1 + disc, i + 1);
    dKwh += q.kwh * fDel / Math.pow(1 + disc, i + 1);
  });
  return {
    capex: Math.round(capex),
    pvCapex: Math.round(pvCapex),
    battCapex: Math.round(battCapex),
    kYield: scR(kY, 4),
    split: sp,
    deliveredKwh: Math.round(totalDel),
    rows,
    years: yrs,
    payback,
    npv: Math.round(npv),
    irr: irr == null ? null : scR(irr * 100, 2),
