/* ============================================================
   flash+solar — I-V Curve · ชดเชยสภาพอากาศ · ROI
   ------------------------------------------------------------
...
/* ── ตารางช่วยหาตัวบัง ──
   เดิมทุกจุดที่สุ่มบนแผงต้องลองยิงใส่ตัวบังทุกตัว = แผง × ตัวบัง
   ผังเล็กไม่รู้สึก แต่ผังใหญ่ (แผงหลักพัน) กลายเป็นสิบล้านคู่ต่อหนึ่งช่วงเวลา
   คูณ 168 ช่วงเวลาทั้งปี = จอค้างเป็นนาที (เคสจริง 4588 แผง ค้างราว 5 นาที)

   ตรงนี้ฉายตัวบังลงระนาบที่ตั้งฉากกับลำแสง แล้วแบ่งเป็นช่องตาราง
   ลำแสงจากจุดหนึ่งจะโดนตัวบังได้ก็ต่อเมื่อเงาของทั้งคู่ทับกันบนระนาบนี้
   จุดหนึ่งจึงลองเฉพาะตัวบังที่ตกอยู่ในช่องเดียวกับตัวเอง — ผลลัพธ์เท่าเดิมทุกตัวเลข
   (ตัวบังถูกใส่ไว้ทุกช่องที่วงกลมรัศมี br ของมันแตะ จึงไม่มีทางตกหล่น)
   ตัวบังที่ใหญ่กว่าช่องมาก ๆ (ผิวหลังคาทั้งผืน) ไม่ต้องลงตาราง เก็บไว้ลองทุกจุดเลยจะคุ้มกว่า */
function ivShadeGrid(B, dir) {
  /* แกนสองแกนบนระนาบที่ตั้งฉากกับลำแสง */
  const up = Math.abs(dir.y) > 0.9 ? { x: 1, y: 0, z: 0 } : { x: 0, y: 1, z: 0 };
  let ux = dir.y * up.z - dir.z * up.y, uy = dir.z * up.x - dir.x * up.z, uz = dir.x * up.y - dir.y * up.x;
  const ul = Math.sqrt(ux * ux + uy * uy + uz * uz) || 1;
  ux /= ul; uy /= ul; uz /= ul;
  const vx = dir.y * uz - dir.z * uy, vy = dir.z * ux - dir.x * uz, vz = dir.x * uy - dir.y * ux;

  /* ขนาดช่อง = ประมาณขนาดตัวบังทั่วไป (แผงหนึ่งใบ) จะได้ไม่ละเอียดหรือหยาบเกินไป */
  let med = 0;
  for (let k = 0; k < B.length; k++) med += B[k].br || 0;
  const cell = Math.max(0.5, Math.min(6, (B.length ? med / B.length : 1) * 0.6));
  const big = [], map = new Map();
  /* ตัวบังที่ไม่ได้ลงตาราง จะตกไปอยู่กลุ่ม "ใหญ่" ซึ่งถูกเทียบกับ *ทุกจุดของทุกแผง*
     ผิวหลังคาผืนหนึ่งกินพื้นที่ทั้งหลัง รัศมีครอบจึงหลายสิบเมตร เข้าเกณฑ์ "ใหญ่" ทุกผืน
     หลังคาโรงงาน 10 ผืน × แผง 4588 ใบ × 5 จุด = เทียบเปล่า ๆ แสนกว่าครั้งต่อหนึ่งทิศแสง
     ทั้งที่ผิวหลังคาส่วนใหญ่อยู่คนละมุมโรงงานกับจุดที่กำลังคิด

     ลงตารางให้ได้มากที่สุดจึงคุ้มกว่าเสมอ — เกณฑ์เลยเปลี่ยนจาก "รัศมีใหญ่ไหม"
     เป็น "ลงตารางแล้วกินกี่ช่อง" ก้อนใหญ่แต่มีไม่กี่ก้อนก็ยังลงไหว
     ผลลัพธ์เท่าเดิมเป๊ะ ตารางเป็นแค่ตัวกรองหยาบ ๆ ว่า "ก้อนไหนมีสิทธิ์บังจุดนี้บ้าง"
     ก้อนที่ลงตารางยังถูกทดสอบด้วยเรขาคณิตชุดเดิมทุกประการ */
  const SPREAD = 40000;          // ช่องมากสุดที่ยอมให้ตัวบังหนึ่งก้อนกิน
  let budget = 900000;          // เพดานรวมทั้งตาราง กันผังหลุดโลกกินหน่วยความจำ
  for (let k = 0; k < B.length; k++) {
    const b = B[k], br = b.br || 0;
    const span = Math.floor(2 * br / cell) + 2;
    const need = span * span;
    if (need > SPREAD || need > budget) { big.push(b); continue; }
    budget -= need;
    const cu = b.bc.x * ux + b.bc.y * uy + b.bc.z * uz;
    const cv = b.bc.x * vx + b.bc.y * vy + b.bc.z * vz;
    const i0 = Math.floor((cu - br) / cell), i1 = Math.floor((cu + br) / cell);
    const j0 = Math.floor((cv - br) / cell), j1 = Math.floor((cv + br) / cell);
    for (let i = i0; i <= i1; i++) {
      for (let j = j0; j <= j1; j++) {
  ...
  return { ux: ux, uy: uy, uz: uz, vx: vx, vy: vy, vz: vz, cell: cell, map: map, big: big };
}
...
function ivShadeAt(geo, dir) {
  ...
  for (let i = 0; i < geo.panels.length; i++) {
    ...
    for (let s = 0; s < S.length; s++) {
      const o = S[s];
      /* ตัวบังที่มีสิทธิ์บังจุดนี้ = ตัวที่อยู่ช่องเดียวกันบนระนาบตั้งฉากลำแสง + ตัวใหญ่ที่ไม่ได้ลงตาราง */
      const key = Math.floor((o.x * G.ux + o.y * G.uy + o.z * G.uz) / G.cell) * 4194304
                + Math.floor((o.x * G.vx + o.y * G.vy + o.z * G.vz) / G.cell);
