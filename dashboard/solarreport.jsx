/* ============================================================
   ...
   สีในไฟล์นี้เขียนเป็นรหัสสีตรง ๆ ไม่ใช้ var(--…) เพราะหน้าต่างรายงานเป็นเอกสารคนละใบ
   ไม่ได้โหลด CSS ของแอป ตัวแปรจึงไม่มีค่า → fill/stroke ตกกลับเป็นสีดำหมด
   (สีแดงในแผนภาพค่าสูญเสียเคยกลายเป็นสีดำเพราะเรื่องนี้)
   ส่วนสีที่หลุดมาจากไฟล์อื่น (เช่นชุดสีสตริงใน solarui) กันไว้อีกชั้นด้วย :root ใน RP_CSS
   ============================================================ */
...
const rpLogoURL = () => { try { return new URL((window.BRANDING || {}).docMark || "dashboard/assets/flashplussolar-mark.png", location.href).href; } catch (e) { return ""; } };
...
const RP_I18N = {
  ...
  "ตรวจแรงดันครบทั้งสองด้าน: ตอนอากาศเย็น {}°C แรงดันวงจรเปิดต้องไม่เกินพิกัดอินเวอร์เตอร์ · ตอนแผงร้อน {}°C แรงดันทำงานต้องไม่หลุดต่ำกว่าช่วง MPPT · DC/AC = ":
    ["Voltage verified at both extremes: at a cold {}°C the open-circuit voltage must stay within the inverter rating · at a hot panel {}°C the operating voltage must not drop below the MPPT window · DC/AC = ",
     "两端极限电压均已校核：低温 {}°C 时开路电压不得超过逆变器额定值 · 组件高温 {}°C 时工作电压不得低于 MPPT 区间 · DC/AC = "],

  /* โน้ตอธิบายว่าสัดส่วนใช้ไฟเองมาจากไหน — ท้ายหัวข้อการใช้ไฟ */
  "ตัวเลข “ใช้เอง {}%” นี้ไม่ได้ตั้งขึ้นเอง แต่มาจากการจำลองเทียบชั่วโมงต่อชั่วโมงตลอดทั้งปี และถูกนำไปใช้คิดผลตอบแทนในหัวข้อถัดไปโดยตรง — หน่วยที่ใช้เองมีมูลค่าเท่าค่าไฟเต็ม ส่วนที่ขายคืนได้ราคาต่ำกว่า":
    ["The “{}% self-consumed” figure is not a typed-in setting: it comes from an hour-by-hour simulation across the whole year and feeds the return calculation in the next section directly — every self-consumed unit is worth the full retail tariff, while exported units earn less",
     "“自用 {}%” 并非人工设定，而是全年逐小时模拟的结果，并直接用于下一章节的收益计算 —— 自用电量按全额电价计值，上网电量单价较低"],
  " และหน่วยที่ถูกตัดทิ้งไม่มีมูลค่าเลย":
    [", and curtailed units are worth nothing at all", "，而被削减的电量没有任何价值"],

  "เงินลงทุน": ["Investment", "投资额"],
  "คืนทุนปีที่ {}": ["Payback in year {}", "第 {} 年回本"],
  "สมมติฐาน: ค่าไฟ {} บาท/หน่วย ปรับขึ้นปีละ {}% · ":
    ["Assumptions: tariff {} THB/unit rising {}% per year · ",
     "假设条件：电价 {} 泰铢/度，每年上涨 {}% · "],
  "ใช้ไฟเอง {}% และขายคืน {}% (มาจากการจำลองชั่วโมงต่อชั่วโมงในหัวข้อการใช้ไฟ ไม่ใช่ค่าที่ตั้งเอง)":
    ["self-consumption {}% and export {}% (from the hour-by-hour simulation in the consumption section, not a typed-in figure)",
     "自用 {}%，上网 {}%（来自用电章节的逐小时模拟，非人工设定）"],
  "ใช้ไฟเอง {}% ส่วนที่เหลือ":
    ["self-consumption {}%, the remainder exported", "自用 {}%，其余上网"],
  " ขายคืนหน่วยละ {} บาท · ค่าดูแลรักษาปีละ {}% ของค่าติดตั้ง · อัตราคิดลด {}%":
    [" at {} THB per unit · O&M {}% of installed cost per year · discount rate {}%",
     "，电价每度 {} 泰铢 · 运维费为安装成本的 {}%/年 · 折现率 {}%"],
  ...
  const cover = !P.cover ? "" :
    ...
    '<header class="cover">' +
      '<div class="cv-bar">' +
        ...
        "<div><b>" + RP_ESC(window.BRANDING.legal) + "</b>" +
