...
const rpLogoURL = () => {
  try {
    return new URL("dashboard/assets/phithan-mark.png", location.href).href;
  } catch (e) {
    return "";
  }
};
...
function rpMonthly(data) {
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + data.map((v, i) => {
    ...
    return '<rect x="' + x.toFixed(1) + '" y="' + (H - B - h).toFixed(1) + '" width="' + bw.toFixed(1) + '" height="' + h.toFixed(1) + '" rx="2" fill="#22A35B"/>' + '<text x="' + (x + bw / 2).toFixed(1) + '" y="' + (H - B - h - 4).toFixed(1) + '" text-anchor="middle" font-size="9" font-weight="700" fill="#5B6B63">' + Math.round(v / 100) / 10 + 'k</text>' + '<text x="' + (x + bw / 2).toFixed(1) + '" y="' + (H - 6) + '" text-anchor="middle" font-size="9" fill="#8A968F">' + SC_MON[i].replace(".", "") + "</text>";
  }).join("") + "</svg>";
}
function rpCash(roi) {
  const W = 720,
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + '<line x1="0" y1="' + Y(0).toFixed(1) + '" x2="' + W + '" y2="' + Y(0).toFixed(1) + '" stroke="#C9D3CD" stroke-width="1"/>' + rows.map((r, i) => '<rect x="' + X(i).toFixed(1) + '" y="' + Math.min(Y(r.cum), Y(0)).toFixed(1) + '" width="' + bw.toFixed(1) + '" height="' + Math.max(1, Math.abs(Y(r.cum) - Y(0))).toFixed(1) + '" rx="1.5" fill="' + (r.cum >= 0 ? "#22A35B" : "#C9D3CD") + '"/>').join("") + (roi.payback != null && roi.payback <= rows.length ? '<line x1="' + X(roi.payback).toFixed(1) + '" y1="' + T + '" x2="' + X(roi.payback).toFixed(1) + '" y2="' + (H - B) + '" stroke="var(--tint-amber-tx)" stroke-width="1.4" stroke-dasharray="4 3"/><text x="' + (X(roi.payback) + 5).toFixed(1) + '" y="' + (T + 10) + '" font-size="10" font-weight="700" fill="var(--tint-amber-tx)">คืนทุนปีที่ ' + roi.payback + "</text>" : "") + rows.map((r, i) => i % 5 === 4 || i === 0 ? '<text x="' + (X(i) + bw / 2).toFixed(1) + '" y="' + (H - 5) + '" text-anchor="middle" font-size="9" fill="#8A968F">' + r.year + "</text>" : "").join("") + "</svg>";
}
function rpIv(exp, stcRef, meas) {
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + '<line x1="' + L + '" y1="' + T + '" x2="' + L + '" y2="' + (H - B) + '" stroke="#C9D3CD"/>' + '<line x1="' + L + '" y1="' + (H - B) + '" x2="' + (W - R) + '" y2="' + (H - B) + '" stroke="#C9D3CD"/>' + (stcRef ? '<path d="' + line(stcRef) + '" fill="none" stroke="#A8B4AE" stroke-width="1.1" stroke-dasharray="4 3"/>' : "") + '<path d="' + line(exp) + '" fill="none" stroke="#22A35B" stroke-width="1.9"/>' + '<circle cx="' + X(exp.vmp).toFixed(1) + '" cy="' + Y(exp.imp).toFixed(1) + '" r="3.2" fill="#fff" stroke="#22A35B" stroke-width="1.8"/>' + (meas && meas.voc ? '<circle cx="' + X(meas.voc).toFixed(1) + '" cy="' + Y(0).toFixed(1) + '" r="3.2" fill="#2563EB"/>' : "") + (meas && meas.isc ? '<circle cx="' + X(0).toFixed(1) + '" cy="' + Y(meas.isc).toFixed(1) + '" r="3.2" fill="#2563EB"/>' : "") + (meas && meas.vmp && meas.imp ? '<circle cx="' + X(meas.vmp).toFixed(1) + '" cy="' + Y(meas.imp).toFixed(1) + '" r="3.6" fill="#2563EB" stroke="#fff" stroke-width="1.2"/>' : "") + '<text x="' + (W - R) + '" y="' + (H - 6) + '" text-anchor="end" font-size="8.5" fill="#8A968F">V</text>' + '<text x="' + (L - 4) + '" y="' + (T + 8) + '" text-anchor="end" font-size="8.5" fill="#8A968F">A</text>' + "</svg>";
}
...
function rpDayLight(sim, groups) {
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + '<defs><pattern id="rpHatch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">' + '<rect width="6" height="6" fill="rgba(71,85,105,.16)"/><line x1="0" y1="0" x2="0" y2="6" stroke="#475569" stroke-width="2" opacity=".5"/></pattern></defs>' + [0, 0.25, 0.5, 0.75, 1].map(f => '<line x1="' + L + '" y1="' + Y(top * f).toFixed(1) + '" x2="' + (W - R) + '" y2="' + Y(top * f).toFixed(1) + '" stroke="#E3E8E6" stroke-width="1"/><text x="' + (L - 6) + '" y="' + (Y(top * f) + 3.5).toFixed(1) + '" text-anchor="end" font-size="8.5" fill="#8A968F">' + Math.round(top * f) + "</text>").join("") + '<path d="' + path(r => r.poaAvg) + " " + sim.rows.slice().reverse().map(r => "L" + X(r.h).toFixed(1) + " " + Y(net(r)).toFixed(1)).join(" ") + ' Z" fill="url(#rpHatch)"/>' + '<path d="' + path(net) + " L" + X(sim.rows[sim.rows.length - 1].h).toFixed(1) + " " + Y(0) + " L" + X(sim.rows[0].h).toFixed(1) + " " + Y(0) + ' Z" fill="rgba(34,163,91,.18)"/>' + '<path d="' + path(r => r.ghi) + '" fill="none" stroke="#A8B4AE" stroke-width="1.2" stroke-dasharray="5 4"/>' + '<path d="' + path(net) + '" fill="none" stroke="#22A35B" stroke-width="2"/>' + ticks.map(h => '<text x="' + X(h).toFixed(1) + '" y="' + (H - B + 13) + '" text-anchor="middle" font-size="8.5" fill="#8A968F">' + h + ":00</text>").join("") + '<text x="' + (L - 6) + '" y="' + (T - 2) + '" text-anchor="end" font-size="8" fill="#8A968F">W/m²</text></svg>' + '<p class="legend"><b style="color:#A8B4AE">┅</b> แสงบนพื้นราบ &nbsp;&nbsp; <b style="color:#22A35B">━</b> แสงบนหน้าแผงจริง (หลังหักเงา)' + (sim.shadeFrom != null ? ' &nbsp;&nbsp; <b style="color:#475569">▨</b> ส่วนที่เงาบังกินไป' : "") + "</p>" + (sim.shadeFrom != null ? '<table class="strip">' + (groups || []).map(g => "<tr><td>" + RP_ESC(g.roofName + (g.side ? " · " + g.side : "")) + "</td><td>" + '<span class="bar">' + sim.rows.map(r => {
    const v = r.per[g.key] ? r.per[g.key].shade : 0;
    const c = v <= 0.5 ? "#EDF1EF" : v < 15 ? "var(--tint-amber-bd)" : v < 40 ? "#F59E0B" : "var(--tint-red-tx2)";
    return '<i style="background:' + c + '"></i>';
  }).join("") + "</span></td></tr>").join("") + "</table>" + '<p class="legend"><span style="float:left">' + ivHM(h0) + '</span><span style="float:right">' + ivHM(h1) + "</span>" + '<b style="color:#EDF1EF">■</b> ไม่มีเงา &nbsp; <b style="color:var(--tint-amber-bd)">■</b> บังบางส่วน &nbsp; ' + '<b style="color:#F59E0B">■</b> บังมาก &nbsp; <b style="color:var(--tint-red-tx2)">■</b> บังเกือบหมด</p>' : "");
}
function rpDayPower(mo, acKw) {
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + [0, 0.5, 1].map(f => '<line x1="' + L + '" y1="' + Yp(pTop * f).toFixed(1) + '" x2="' + (W - R) + '" y2="' + Yp(pTop * f).toFixed(1) + '" stroke="#E3E8E6" stroke-width="1"/>' + '<text x="' + (L - 6) + '" y="' + (Yp(pTop * f) + 3.5).toFixed(1) + '" text-anchor="end" font-size="8.5" font-weight="700" fill="#8A968F">' + Math.round(pTop * f * 10) / 10 + "</text>" + '<text x="' + (W - R + 6) + '" y="' + (Yt(tTop * f) + 3.5).toFixed(1) + '" font-size="8.5" font-weight="700" fill="#C4342B">' + Math.round(tTop * f) + "</text>").join("") + '<text x="' + (L - 6) + '" y="' + (T - 14) + '" text-anchor="end" font-size="8" font-weight="700" fill="#8A968F">kW</text>' + '<text x="' + (W - R + 6) + '" y="' + (T - 14) + '" font-size="8" font-weight="700" fill="#C4342B">°C</text>' + (acKw > 0 && acKw < pTop ? '<line x1="' + L + '" y1="' + Yp(acKw).toFixed(1) + '" x2="' + (W - R) + '" y2="' + Yp(acKw).toFixed(1) + '" stroke="var(--tint-amber-tx)" stroke-width="1.2" stroke-dasharray="5 3"/><text x="' + (W - R - 2) + '" y="' + (Yp(acKw) - 4).toFixed(1) + '" text-anchor="end" font-size="8.5" font-weight="700" fill="var(--tint-amber-tx)">เพดานอินเวอร์เตอร์ ' + acKw + " kW</text>" : "") + '<path d="' + path(cs, c => c.dc, Yp) + " L" + X(cs[cs.length - 1].h).toFixed(1) + " " + Yp(0) + " L" + X(cs[0].h).toFixed(1) + " " + Yp(0) + ' Z" fill="rgba(34,163,91,.13)"/>' + '<path d="' + path(cs, c => c.dc, Yp) + '" fill="none" stroke="#22A35B" stroke-width="1.2" stroke-dasharray="4 3"/>' + '<path d="' + path(cs, c => c.ac, Yp) + '" fill="none" stroke="#0F7A43" stroke-width="2.1" stroke-linejoin="round"/>' + (ts.length > 1 ? '<path d="' + path(ts, c => c.tCell, Yt) + '" fill="none" stroke="#C4342B" stroke-width="1.5" stroke-linejoin="round"/>' : "") + '<circle cx="' + X(pk.h).toFixed(1) + '" cy="' + Yp(pk.ac).toFixed(1) + '" r="3.2" fill="#fff" stroke="#0F7A43" stroke-width="1.8"/>' + '<text x="' + clamp(X(pk.h), L + 36, W - R - 36).toFixed(1) + '" y="' + (Yp(pk.ac) - 7).toFixed(1) + '" text-anchor="middle" font-size="9.5" font-weight="700" fill="#0F7A43">สูงสุด ' + Math.round(pk.ac * 100) / 100 + " kW</text>" + '<circle cx="' + X(tk.h).toFixed(1) + '" cy="' + Yt(tk.tCell).toFixed(1) + '" r="3" fill="#fff" stroke="#C4342B" stroke-width="1.6"/>' + '<text x="' + clamp(X(tk.h), L + 32, W - R - 32).toFixed(1) + '" y="' + (Yt(tk.tCell) - 6).toFixed(1) + '" text-anchor="middle" font-size="9" font-weight="700" fill="#C4342B">ร้อนสุด ' + Math.round(tk.tCell) + "°C</text>" + [6, 8, 10, 12, 14, 16, 18].filter(h => h >= h0 && h <= h1).map(h => '<text x="' + X(h).toFixed(1) + '" y="' + (H - B + 13) + '" text-anchor="middle" font-size="8.5" fill="#8A968F">' + h + ":00</text>").join("") + "</svg>" + '<p class="legend"><b style="color:#0F7A43">━</b> กำลังไฟที่ออกจากอินเวอร์เตอร์ (AC) &nbsp;&nbsp; ' + '<b style="color:#22A35B">┅</b> กำลังไฟจากแผง (DC) &nbsp;&nbsp; <b style="color:#C4342B">━</b> อุณหภูมิเซลล์ (แกนขวา)</p>';
}
...
function rpIvAll(curves, stcRef) {
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + Array.from({
    ...
    return '<line x1="' + L + '" y1="' + y.toFixed(1) + '" x2="' + (W - R) + '" y2="' + y.toFixed(1) + '" stroke="#E3E8E6" stroke-width="1"/>' + '<text x="' + (L - 7) + '" y="' + (y + 3.5).toFixed(1) + '" text-anchor="end" font-size="8.5" font-weight="700" fill="#8A968F">' + Math.round(iTop * (1 - k / gI) * 10) / 10 + "</text>" + '<text x="' + (W - R + 7) + '" y="' + (y + 3.5).toFixed(1) + '" font-size="8.5" font-weight="700" fill="var(--tint-amber-tx)">' + (() => {
      const p = pTop * (1 - k / gI);
...
function rpLossFlow(chain) {
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + '<defs><linearGradient id="rpFlowG" x1="0" y1="0" x2="0" y2="1">' + '<stop offset="0%" stop-color="#3ECF84"/><stop offset="55%" stop-color="#22A35B"/><stop offset="100%" stop-color="#0B6B3A"/></linearGradient></defs>' + '<path d="' + trunk + '" fill="url(#rpFlowG)"/>' + seg.map(s => (s.r.kind === "loss" || s.r.kind === "gain") && Math.abs(s.wA - s.wB) > 0.2 ? '<line x1="' + X0 + '" y1="' + ((s.y0 + s.y1) / 2).toFixed(1) + '" x2="' + (X0 + Math.min(s.wA, s.wB)).toFixed(1) + '" y2="' + ((s.y0 + s.y1) / 2).toFixed(1) + '" stroke="#fff" stroke-width="1" stroke-opacity=".28"/>' : "").join("") + seg.map(s => {
    const R = s.r,
      yc = (s.y0 + s.y1) / 2;
    ...
    if (R.kind === "loss" || R.kind === "gain") {
      ...
      return ribbon + (zero ? '<line x1="' + (X0 + s.wA).toFixed(1) + '" y1="' + yc.toFixed(1) + '" x2="' + XL + '" y2="' + yc.toFixed(1) + '" stroke="#D8E0DB" stroke-width="1" stroke-dasharray="2 4"/>' : "") + '<text x="' + LX + '" y="' + (yc - 9).toFixed(1) + '" font-size="10" font-weight="700" fill="' + (zero ? "#8A968F" : "#3A4A43") + '">' + RP_ESC(cut(R.label, 66)) + "</text>" + '<text x="' + LX + '" y="' + (yc + 3).toFixed(1) + '" font-size="9.5" font-weight="800" fill="' + (zero ? "#8A968F" : gain ? "#1D4ED8" : heavy ? "var(--tint-red-tx)" : "var(--tint-amber-tx)") + '">' + RP_ESC(gain ? "+" + R.pct + "%  ·  " + rpN(R.gain) + " kWh" : zero ? "ไม่เสียพลังงานในด่านนี้" : "−" + R.pct + "%  ·  " + rpN(R.loss) + " kWh") + (R.unit ? '<tspan fill="#8A968F" font-weight="700">' + RP_ESC("   → " + R.unit) + "</tspan>" : "") + "</text>" + (R.note ? '<text x="' + LX + '" y="' + (yc + 14).toFixed(1) + '" font-size="8.5" font-weight="600" fill="#8A968F">' + RP_ESC(cut(R.note, 70)) + "</text>" : "");
    }
    const big = R.kind === "end",
      first = R.kind === "start";
    const accent = big ? "#0F7A43" : first ? "var(--tint-amber-tx)" : "#C9D3CD";
    return '<line x1="' + X0 + '" y1="' + s.y1.toFixed(1) + '" x2="' + (XL + 4) + '" y2="' + s.y1.toFixed(1) + '" stroke="' + (big ? "#0F7A43" : "#D8E0DB") + '" stroke-width="' + (big ? 1.4 : 1) + '"' + (big ? ' stroke-opacity=".7"' : ' stroke-dasharray="3 4"') + "/>" + '<rect x="' + (LX - 9) + '" y="' + (s.y1 - 32).toFixed(1) + '" width="3" height="30" rx="1.5" fill="' + accent + '" fill-opacity="' + (big || first ? 1 : 0.5) + '"/>' + '<text x="' + LX + '" y="' + (s.y1 - 23).toFixed(1) + '" font-size="10.5" font-weight="800" fill="#16211D">' + RP_ESC(cut(R.label, 60)) + "</text>" + '<text x="' + LX + '" y="' + (s.y1 - 11).toFixed(1) + '" font-size="9.5" font-weight="800" fill="' + (big ? "#0F7A43" : first ? "var(--tint-amber-tx)" : "#3A4A43") + '">' + RP_ESC(rpN(R.kwh) + " kWh") + (R.unit ? '<tspan fill="#8A968F" font-weight="700">' + RP_ESC("   ·   " + R.unit) + "</tspan>" : "") + (big ? '<tspan fill="#0F7A43" font-weight="800">' + RP_ESC("   ·   PR " + R.pct + "%") + "</tspan>" : "") + "</text>" + (R.note ? '<text x="' + LX + '" y="' + (s.y1 - 1).toFixed(1) + '" font-size="8.5" font-weight="600" fill="#8A968F">' + RP_ESC(cut(R.note, 70)) + "</text>" : "");
  }).join("") + "</svg>";
}
const RP_ISO = [{
  at: 0.40,
...
function rpSunPath(path, iso) {
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + '<rect x="' + L + '" y="' + T + '" width="' + (W - L - R) + '" height="' + (H - T - B) + '" rx="6" fill="#F6F8F7"/>' + (iso ? iso.cells.map(c => {
    ...
    return p.segs.map(sg => '<path d="' + line(sg) + '" fill="none" stroke="var(--tint-amber-tx)" stroke-width="' + (main ? 1.9 : 1.2) + '" opacity="' + (main ? 0.95 : 0.6) + '" stroke-linecap="round"/>').join("");
  }).join("") + [path.paths[0], path.paths[path.paths.length - 1]].map((p, i) => p && p.peak ? '<text x="' + Math.max(L + 36, Math.min(W - R - 36, X(p.peak.az))).toFixed(1) + '" y="' + (Y(p.peak.alt) + (i ? 13 : -7)).toFixed(1) + '" text-anchor="middle" font-size="8.5" font-weight="800" fill="var(--tint-amber-tx)">' + RP_ESC(p.label) + "</text>" : "").join("") + "</svg>" + (iso ? '<p class="note"><b>ระดับสีของเงาบัง</b> — ' + RP_ISO.slice().reverse().map(b => b.lb).join(" · ") + " (เข้มขึ้น = โดนบังหนักขึ้น) · เส้นทางเดินเส้นไหนวิ่งผ่านพื้นที่สี แปลว่าเดือนนั้นเวลานั้นแผงโดนเงาบังแน่นอน</p>" : "");
}
...
function rpIvFamily(curves, mode) {
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + Array.from({
    ...
    const y = T + (H - T - B) * k / 4,
      p = pTop * (1 - k / 4);
    return '<line x1="' + L + '" y1="' + y.toFixed(1) + '" x2="' + (W - R) + '" y2="' + y.toFixed(1) + '" stroke="#E3E8E6" stroke-width="1"/>' + '<text x="' + (L - 7) + '" y="' + (y + 3.5).toFixed(1) + '" text-anchor="end" font-size="8.5" font-weight="700" fill="#8A968F">' + Math.round(iTop * (1 - k / 4) * 10) / 10 + "</text>" + '<text x="' + (W - R + 7) + '" y="' + (y + 3.5).toFixed(1) + '" font-size="8.5" font-weight="700" fill="var(--tint-amber-tx)">' + (p >= 1000 ? Math.round(p / 100) / 10 + "k" : Math.round(p)) + "</text>";
  }).join("") + Array.from({
    length: 6
  }).map((_, k) => {
    const v = vTop * k / 5;
    return (k ? '<line x1="' + X(v).toFixed(1) + '" y1="' + T + '" x2="' + X(v).toFixed(1) + '" y2="' + (H - B) + '" stroke="#EEF1F0" stroke-width="1"/>' : "") + '<text x="' + X(v).toFixed(1) + '" y="' + (H - B + 13) + '" text-anchor="middle" font-size="8.5" fill="#8A968F">' + Math.round(v) + "</text>";
  }).join("") + '<line x1="' + L + '" y1="' + T + '" x2="' + L + '" y2="' + (H - B) + '" stroke="#C9D3CD" stroke-width="1.1"/>' + '<text x="' + (L - 7) + '" y="' + (T - 5) + '" text-anchor="end" font-size="8" font-weight="700" fill="#8A968F">A</text>' + '<text x="' + (W - R + 7) + '" y="' + (T - 5) + '" font-size="8" font-weight="700" fill="var(--tint-amber-tx)">W</text>' + '<text x="' + (W - R) + '" y="' + (H - 4) + '" text-anchor="end" font-size="8.5" fill="#8A968F">แรงดัน (V)</text>' + '<path d="' + list.map((c, k) => (k ? "L" : "M") + X(c.vmp).toFixed(1) + " " + Yi(c.imp).toFixed(1)).join(" ") + '" fill="none" stroke="#A8B4AE" stroke-width="1" stroke-dasharray="2 4"/>' + list.map((c, i) => '<path d="' + pathOf(c, Yp, q => q.p) + '" fill="none" stroke="' + colOf(i) + '" stroke-width="1.3" stroke-dasharray="4 3" opacity=".7"/>' + '<path d="' + pathOf(c, Yi, q => q.i) + '" fill="none" stroke="' + colOf(i) + '" stroke-width="2" stroke-linejoin="round"/>' + '<circle cx="' + X(c.vmp).toFixed(1) + '" cy="' + Yi(c.imp).toFixed(1) + '" r="3.4" fill="#fff" stroke="' + colOf(i) + '" stroke-width="1.9"/>').join("") + (() => {
    const rh = 12.5,
      cw = 214,
      pad = 8;
...
function rpPxx(px, mode) {
  ...
  const fill = under.length ? '<path d="M' + under[0].x.toFixed(1) + " " + Y(0).toFixed(1) + " L" + under.map(p => p.x.toFixed(1) + " " + p.y.toFixed(1)).join(" L") + " L" + under[under.length - 1].x.toFixed(1) + " " + Y(0).toFixed(1) + ' Z" fill="var(--tint-amber-tx)" opacity=".16"/>' : "";
  ...
  return '<svg viewBox="0 0 ' + W + " " + H + '" class="chart">' + '<line x1="' + L + '" y1="' + Y(0).toFixed(1) + '" x2="' + (W - R) + '" y2="' + Y(0).toFixed(1) + '" stroke="#D8E0DC" stroke-width="1.2"/>' + fill + '<polyline points="' + pts.map(p => p.x.toFixed(1) + "," + p.y.toFixed(1)).join(" ") + '" fill="none" stroke="#0B5F35" stroke-width="2"/>' + mark(p50, 50, "#0B5F35") + mark(p90, 90, "var(--tint-amber-tx)") + '<text x="' + X(p90).toFixed(1) + '" y="' + (H - 4) + '" text-anchor="middle" font-size="8.5" font-weight="800" fill="var(--tint-amber-tx)">โอกาสตกลงมาต่ำกว่านี้ 10%</text>' + '<text x="' + L + '" y="' + (H - 4) + '" font-size="8.5" font-weight="700" fill="#8A968F">แย่กว่าที่คิด</text>' + '<text x="' + (W - R) + '" y="' + (H - 4) + '" text-anchor="end" font-size="8.5" font-weight="700" fill="#8A968F">ดีกว่าที่คิด</text>' + "</svg>";
}
const RP_SECTIONS = [{
  key: "cover",
  label: "หน้าปก",
...
function suReportHTML(D) {
  ...
  const ivAllSec = ivCurves.length && P.ivAll ? "<h3>ค่าที่ควรวัดได้ของทุก" + (D.isMicro ? "ไมโคร" : "สตริง") + " ณ " + ivHM(D.simHour) + " น.</h3>" + rpTable([D.isMicro ? "ไมโคร" : "สตริง", "แผง"].concat(D.isMicro ? ["ต่อช่อง"] : []).concat(["แสง W/m²", "เซลล์ °C", "Voc", "Isc", "Vmp", "Imp", D.isMicro ? "Pmax/ช่อง" : "Pmax"]).concat(D.isMicro ? ["รวมทั้งตัว"] : []), (D.ivRows || []).filter(r => r.a && r.a.exp).map((r, i) => {
    const ch = Math.max(1, Math.round((r.u.count || 1) / Math.max(1, r.u.n)));
    return [{
      html: '<span class="dot" style="background:' + (typeof suColor === "function" ? suColor(r.u.sid || i + 1) : "#22A35B") + '"></span><b>' + RP_ESC(r.u.name) + "</b>"
