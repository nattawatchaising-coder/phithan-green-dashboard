...
/* คนนี้อนุมัติ "ใบนี้" ได้ไหม — คืนเหตุผลกลับไปด้วย หน้าจอจะได้บอกได้ว่าทำไมกดไม่ได้
   ค่าเริ่มต้นคือห้ามอนุมัติใบของตัวเอง แม้แอดมิน — คนเบิกกับคนตรวจต้องคนละคน
   บริษัทที่คุมเงินอยู่คนเดียวจริง ๆ เปิดสวิตช์ selfApprove เป็นรายบัญชีได้ในหน้าจัดการผู้ใช้
   เปิดแล้ววงเงินอนุมัติยังคุมอยู่ และประวัติยังบันทึกว่าใครกด ตรวจย้อนได้ว่าใครอนุมัติใบตัวเอง */
function ecApproveCheck(claim, user, role) {
  ...
  if (claim.byId && claim.byId === user.id && !user.selfApprove) {
    return { ok: false, why: "อนุมัติใบของตัวเองไม่ได้ — ต้องให้คนอื่นอนุมัติ" };
  }
